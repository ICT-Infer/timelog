...
class EntryAdmin (admin.ModelAdmin):
  ...
  def get_form (self, req, obj, **kwargs):
    if obj is not None and not hasattr(obj, 'tz_maniped'):
      ...
      obj.t_begin = obj.t_begin.astimezone(pytz.timezone(obj.tz_begin)).replace(tzinfo=None)
      obj.t_begin = timezone.make_aware(obj.t_begin, timezone.get_current_timezone())

      if obj.t_end:
        obj.t_end = obj.t_end.astimezone(pytz.timezone(obj.tz_end)).replace(tzinfo=None)
